...
function initialise_model_weights(layer_dims::Vector{Int}, seed::Number=1)::Dict
    ...
    for jj in 2:eachindex(layer_dims)
        params[string("w", jj - 1)] = rand(StableRNG(seed), layer_dims[jj]) * sqrt(2 / layer_dims[jj])
        params[string("b", jj - 1)] = zeros(layer_dims[jj])
    end
    return params
end
"""
...
"""
relu_backwards(∂A, activated_cache) = ∂A .* (activated_cache .> 0)
"""
...
function train_network(layer_dims, DMatrix, Y; η=0.001, epochs=1000, seed=2020, verbose=true)
    ...
    for jj = 1:epochs
        ...
        if verbose
            println("Iteration -> $jj, Cost -> $cost, Accuracy -> $acc")
        end
